...
%Solve system
%_SS_START
% epsilon = speye(numel(antiIndex))*(1e-3);
% x=(L(antiIndex,antiIndex)+epsilon)\b;
% X = gmres(L(antiIndex,antiIndex), b);
L(isnan(L(antiIndex,antiIndex))) = 0;
%_SS_END
x=L(antiIndex,antiIndex)\b;
% x = pinvs(L(antiIndex,antiIndex)) * b;
